...
$(function () {
		...
		success: function (json) {
			...
			if (testList()) {
				...
				for (var i = 0; i < q.list.length; i++) {
					...
					$list.append('<li style="list-style-type:none;"><a>'+(i+1)+'. '+data.name+'</a><br/><small style="float:right">'+data.artist.join('/')+'</small></li><br/><br/>');
				}
	...
	q.load = function (n) {
		...
		$title.html('<strong>正在播放：'+data.name+'</strong><span> - </span><span class="artist">'+data.artist.join('/')+'</span>');
